...
///////findByItems ///////////////////to do
@Get('items')
async findAllUsersByItems(@Query('items') items: string,@Res() response) {
  try {
    const users=await this.usersService.findAllUsersByItems(items);
    return response.status(HttpStatus.OK).json({
      message:'Users found successfully by Items',
